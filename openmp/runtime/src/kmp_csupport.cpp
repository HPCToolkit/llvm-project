...
// FIXME VI3: temporary fix
static __thread bool runtime_caller = false;
/*!
@ingroup PARALLEL
@param loc  source location information
@param global_tid  global thread number
...
void __kmpc_serialized_parallel(ident_t *loc, kmp_int32 global_tid) {
  ...
  runtime_caller = OMPT_FRAME_SET_P(parent_frame, enter);
  if (!runtime_caller) {
...
void __kmpc_end_serialized_parallel(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled.enabled &&
    ...
    if (ompt_enabled.ompt_callback_parallel_end) {
      // TODO VI3: move this to the very end.
  ...
  // FIXME VI3: How to know if the code is generated by the clang?
  if (!runtime_caller) {
    // clear enter frame for the serialized parallel region
    // TODO VI3: What happens when have multiple serialized regions?
    ompt_task_info_t *task_info = OMPT_CUR_TASK_INFO(this_thr);
